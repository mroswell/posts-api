...
#       response = self.client.get("/api/posts/{}".format(postB.id))
        response = self.client.get("/api/posts/{}/delete".format(postB.id),
            headers=[("Accept", "application/json")]
        )
...
#       response = self.client.get("/api/posts/{}".format(postB.id))
        response = self.client.delete("/api/posts/3",
            headers=[("Accept", "application/json")]
        )

        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.mimetype, "application/json")
#        self.assertEqual(response.response ={"message": message})
    ...
    def testGetPostsWithTitleAndBody(self):
        ...
        postA = models.Post(title="Post with bells", body="Just a test of whistles")
        postB = models.Post(title="Post with whistles", body="Still a test")
        postC = models.Post(title="Post with bells and whistles",
                            body="Another test of bells")
        ...
        post = posts[0]
        self.assertEqual(post["title"], "Post with bells and whistles")
        self.assertEqual(post["body"], "Another test of bells")
